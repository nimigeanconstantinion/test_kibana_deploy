...
docker build -f Dockerfile -t test-kibana:%SERVICE_VERSION% .

echo Getting the new image ID...
setlocal enabledelayedexpansion
for /f "tokens=*" %%i in ('docker images -q test-kibana:%SERVICE_VERSION%') do set IMAGE_ID=%%i
echo New image ID: !IMAGE_ID!
endlocal

echo Tagging the image...
docker tag test-kibana:%SERVICE_VERSION%  ion21/test-kibana:%SERVICE_VERSION%
